describe('Tickets page', () => {
  ...
  it('Should check Sort functionality', () => {
    ...
    cy.url()
      .should('eq', 'http://localhost:3000/tickets?_page=1&_limit=8&_sort=ticket&_order=desc')
      .wait(1000);
    cy.get('[data-testid="heading_A"]').should('not.exist');

    cy.get('[data-testid="sort_select"]').click();
    cy.get('[data-testid="item_ticket-asc"]').click();

    cy.url().should('eq', 'http://localhost:3000/tickets?_page=1&_limit=8&_sort=ticket&_order=asc');
    cy.get('[data-testid="heading_A"]').should('exist');
  ...
  it('Should check Filter functionality', () => {
    ...
    cy.url()
      .should(
        'eq',
        'http://localhost:3000/tickets?_page=1&_limit=8&_sort=ticket&_order=asc&priority=High',
      )
      .wait(1000);
    cy.get('[data-testid="heading_A"]').should('exist');
    ...
    cy.url()
      .should(
        'eq',
        'http://localhost:3000/tickets?_page=1&_limit=8&_sort=ticket&_order=asc&priority=High&priority=Low',
      )
      .wait(1000);
    cy.get('[data-testid="heading_A"]').should('exist');
    ...
    cy.url()
      .should(
        'eq',
        'http://localhost:3000/tickets?_page=1&_limit=8&_sort=ticket&_order=asc&priority=Low',
      )
      .wait(1000);
    cy.get('[data-testid="heading_A"]').should('not.exist');
